...
import com.gina.service.UsStockService;
...
public class usmainController {
    @Autowired
    private UsStockService myservice;
    
    @Autowired
    private StockTechCalService calService;
    
    @RequestMapping(value="/usaHome")
    ...
    @RequestMapping(value="/usaOnePage")
    public String showOnePage(String ts_code, Model model){
        UsStockOneMainTopData dt = myservice.getOneMainTopData(ts_code);
        model.addAttribute("basic", dt);
        
        List<UsStockFina> finalist = myservice.getLast2FinaReport(ts_code);
        model.addAttribute("fina", finalist);
        
        StockCalData calData = calService.getUsStockCalData(ts_code, 80);
        ...
        List<UsStockDaily> last11 = myservice.getLastNumUsStockDailyDesc(ts_code, 10);
        UsStockDaily[] last10 = util.setPctChgsAndCleans(last11);
